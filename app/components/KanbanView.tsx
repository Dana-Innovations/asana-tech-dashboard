  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="overflow-x-auto pb-4">
        <div className="inline-flex gap-4 min-h-[calc(100vh-140px)] px-2">
        {columns.map(column => (
          <KanbanColumn
            key={column.id}
            column={column}
...
function KanbanColumn({ column, onMoveProject, onProjectClick }: KanbanColumnProps) {
  ...
  return (
    <div
      ref={drop as any}
      className={`min-h-[600px] w-[320px] flex-shrink-0 rounded-lg border-2 border-dashed transition-colors ${
        column.color
